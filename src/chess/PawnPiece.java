...
public class PawnPiece implements ChessPiece {
    ...
    private void checkForwardMoves(ChessBoard board, ChessPosition myPosition, int newRow, Collection<ChessMove> moves, int direction) {
        ChessPosition forwardOne = new ChessPositionImpl(newRow, myPosition.getColumn());
        ...
        if (board.getPiece(forwardOne) == null) {
            ...
            if (isNewPawnPosition(myPosition) && board.getPiece(new ChessPositionImpl(myPosition.getRow() + (2 * direction), myPosition.getColumn())) == null) {
                moves.add(new ChessMoveImpl(myPosition, new ChessPositionImpl(myPosition.getRow() + (2 * direction), myPosition.getColumn()), null));
            }
        }
    }
    ...
    private void checkDiagonalCaptures(ChessBoard board, ChessPosition myPosition, int newRow, Collection<ChessMove> moves) {
        for (int diagDirection : new int[]{-1, 1}) {
            int newCol = myPosition.getColumn() + diagDirection;
