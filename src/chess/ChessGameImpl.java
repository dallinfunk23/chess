...
public class ChessGameImpl implements ChessGame {
        ...
        for (ChessPosition pos : ChessPositionImpl.getAllPositions()) {
            ...
            if (piece != null && piece.getTeamColor() == enemyColor) {
                ...
                if (piece.getPieceType() == ChessPiece.PieceType.KING) {
                    moves.removeIf(move -> Math.abs(move.getEndPosition().getColumn() - move.getStartPosition().getColumn()) > 1);
                }
    ...
    private boolean canCastle(ChessBoard board, ChessPosition position, TeamColor color, boolean kingSide) {
        ...
        for (int i : range) {
            ChessPosition checkPos = new ChessPositionImpl(position.getRow(), position.getColumn() + (kingSide ? i : -i));
    ...
    public Collection<ChessMove> validMoves(ChessPosition startPosition) {
        ...
        if (piece == null || (piece.getTeamColor() != currentTeamTurn && !board.getTestingMode()))
        ...
        if (piece.getPieceType() == ChessPiece.PieceType.KING) {
            if (canCastle(board, startPosition, piece.getTeamColor(), true))
                validMoves.add(new ChessMoveImpl(startPosition, new ChessPositionImpl(startPosition.getRow(), startPosition.getColumn() + 2), null));
            if (canCastle(board, startPosition, piece.getTeamColor(), false))
                validMoves.add(new ChessMoveImpl(startPosition, new ChessPositionImpl(startPosition.getRow(), startPosition.getColumn() - 2), null));
        }
    ...
    private Collection<ChessMove> checkEnPassantCaptures(ChessPosition startPosition) {
        ...
        if ((currentPiece.getTeamColor() == TeamColor.WHITE && startPosition.getRow() == 5) ||
                (currentPiece.getTeamColor() == TeamColor.BLACK && startPosition.getRow() == 4)) {

            // Check adjacent squares for enemy pawns that moved two squares in the last move
            for (int colDirection : new int[]{-1, 1}) {
                int column = startPosition.getColumn() + colDirection;
                ...
                if (adjPiece != null && adjPiece.getPieceType() == ChessPiece.PieceType.PAWN && adjPiece.getTeamColor() != currentPiece.getTeamColor()) {
                    ...
                    if (lastMove != null && lastMove.getEndPosition().equals(adjPos) &&
                        ...
                        ChessPosition endPosition = new ChessPositionImpl(startPosition.getRow() + direction, startPosition.getColumn() + colDirection);
    ...
    private boolean doesMoveResultInCheck(ChessMove move, ChessPiece piece) {
        ...
        if (piece.getPieceType() == ChessPiece.PieceType.PAWN &&
            ...
            ChessPosition capturedPawnPos = new ChessPositionImpl(move.getStartPosition().getRow() + direction, move.getEndPosition().getColumn());
        ...
        ChessPosition kingPosition = (piece.getPieceType() == ChessPiece.PieceType.KING) ? move.getEndPosition() : findCurrentKingsPosition(piece.getTeamColor());

        // Determine if the king is under attack
        boolean isCheck = isSquareUnderAttack(kingPosition, piece.getTeamColor());
    ...
    private void executeMove(ChessMove move) {
        ...
        if (piece.getPieceType() == ChessPiece.PieceType.PAWN && move.getEndPosition() != null &&
            ...
            ChessPosition capturingPawnPos = new ChessPositionImpl(move.getEndPosition().getRow(), move.getEndPosition().getColumn());
            board.removePiece(capturingPawnPos);

            // Remove the captured piece
            ChessPosition capturedPawnPos = new ChessPositionImpl(move.getStartPosition().getRow(), move.getEndPosition().getColumn());
            board.removePiece(capturedPawnPos);
        }
    ...
    public void makeMove(ChessMove move) throws InvalidMoveException {
        ...
        if (board.getTestingMode())
            currentTeamTurn = board.getPiece(move.getStartPosition()).getTeamColor();
    ...
    public boolean isInStalemate(TeamColor teamColor) {
        ...
        return ChessPositionImpl.getAllPositions().stream().noneMatch(pos -> {
            ChessPiece piece = board.getPiece(pos);
            return piece != null && piece.getTeamColor() == teamColor && !validMoves(pos).isEmpty();
        });
    }
